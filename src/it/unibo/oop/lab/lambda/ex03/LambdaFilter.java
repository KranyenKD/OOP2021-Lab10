...
public final class LambdaFilter extends JFrame {
    ...
    private enum Command {
    	...
    	COUNT_CHAR("Count the number of chars",s-> String.valueOf(s.length())),
    	COUNT_LINES("Count the number of lines",s-> String.valueOf(s.chars().filter(i-> i == '\n').count()+1)),
    	WORDS_LIST("List all the words in alphabetical order",s->Arrays.stream(s.split(ANY_NON_WORD)).sorted().collect(Collectors.joining("\n"))),
    	WORDS_COUNT("Count the words", s->Arrays.stream(s.split(ANY_NON_WORD)).collect(Collectors.groupingBy(Function.identity(),Collectors.counting()))
    			.entrySet().stream().map(i -> i.getKey() + " -> " + i.getValue()).collect(Collectors.joining("\n"))
    			);
